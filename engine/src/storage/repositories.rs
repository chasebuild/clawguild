...
use uuid::Uuid;
use crate::models::{Agent, Deployment, Team, Task, AgentStatus, DeploymentStatus, TaskStatus};
...
impl AgentRepository {
    ...
    pub async fn create(&self, agent: &Agent) -> Result<()> {
        let record: Option<Agent> = self.db
    ...
    pub async fn get_by_id(&self, id: Uuid) -> Result<Option<Agent>> {
        let agent: Option<Agent> = self.db
            .select(("agents", id.to_string()))
            .await?;
...
impl DeploymentRepository {
    ...
    pub async fn create(&self, deployment: &Deployment) -> Result<()> {
        let record: Option<Deployment> = self.db
    ...
    pub async fn update_status(&self, id: Uuid, status: DeploymentStatus) -> Result<()> {
        let _: Option<Deployment> = self.db
    ...
    pub async fn get_by_agent_id(&self, agent_id: Uuid) -> Result<Option<Deployment>> {
        let mut result = self.db
            .query("SELECT * FROM deployments WHERE agent_id = $agent_id LIMIT 1")
...
impl TeamRepository {
    ...
    pub async fn create(&self, team: &Team) -> Result<()> {
        let record: Option<Team> = self.db
    ...
    pub async fn get_by_id(&self, id: Uuid) -> Result<Option<Team>> {
        let team: Option<Team> = self.db
            .select(("teams", id.to_string()))
            .await?;
...
impl TaskRepository {
    ...
    pub async fn create(&self, task: &Task) -> Result<()> {
        let record: Option<Task> = self.db
